...
def main():
    ...
    for k in [5, 10, 20]:
        ...
        if p is not None:
            overall_pk[f"p_at_{k}"] = round(float(p), 4)
    ...
    for repo in ood_repos:
        ...
        repo_info = {"f1": round(float(repo_f1), 4), "rows": int(n_total),
                     "positive_rate": round(float(n_pos / n_total), 4)}
        for k in [5, 10]:
            p = precision_at_k(repo_y, repo_prob, k)
            if p is not None:
                repo_info[f"p_at_{k}"] = round(float(p), 4)
                print(f"  P@{k:<3d} = {p:.4f}  ({int(p * k)}/{k} correct)")
            else:
                print(f"  P@{k:<3d} = N/A (only {n_total} rows)")
        per_repo_results[repo] = repo_info
    ...
    metrics = {
        "ood_f1": round(float(f1), 4),
        "ood_p_at_5": round(float(ood_p5), 4),
        "ood_p_at_5_pass": bool(ood_p5 >= 0.55),
        "overall": overall_pk,
